...
class Migration:
    ...
    def run(self):
        for item in self.generate_all_comment_views():
            pk = {k: item[k] for k in ('partitionKey', 'sortKey')}
            logger.warning(f'Deleting comment view `{pk}`')
            self.dynamo_table.delete_item(Key=pk)

    def generate_all_comment_views(self):
        ...
        scan_kwargs = {
            ...
            'ExpressionAttributeValues': {':pk_prefix': 'comment/', ':sk_prefix': 'view/'},
        }
        while True:
